...
# This is a new version of the experiment in which we will use only one prior, one delay and we will manipulate t
# the level of adaptation with the 
...
# Stims
fixation = visual.PatchStim(win=win, mask='gauss', size=1, pos=[0,0], sf=0, color=[-1,-1,-1],units= "deg")
fixation_ct = visual.PatchStim(win=win, mask='gauss', size=1, pos=[0,0], sf=0, color=[-0.75,-0.75,-0.75],units="deg")

stim_LC = visual.Circle(win,radius=0.7,pos=[0,0],fillColor=[0.3,0.3,0.3],units= "deg")
#stim_LC.draw()
#win.flip()

stim_HC = visual.Circle(win,radius=0.7,pos=[0,0],fillColor=[0.9,0.9,0.9],units= "deg")
#stim_HC.draw()
#win.flip()

resp = visual.Circle(win,radius=0.7,pos=[0,0],fillColor='red',units= "deg")
ring = visual.Circle(win,radius=st_prop['radius'],pos=[0,0],edges = 60, lineColor='black',units= "deg", fillColor = [0,0,0], interpolate = True)
...
#gaussian_L = 40*gkern(1000, 4)
#gabor_L = visual.ImageStim(win,image=gaussian_L,size=(200, 200), pos=(0, 0), interpolate=True)
#gabor_L.draw()
#win.flip()

#gaussian_H = 100*gkern(1000, 4)
#gabor_H = visual.ImageStim(win,image=gaussian_H,size=(200, 200), pos=(0, 0), interpolate=True)
#gabor_H.draw()
#win.flip()


#plt.figure()
#plt.plot(np.linspace(1, np.shape(gaussian_L)[0],1000), gaussian_L[500,:])
#plt.plot(np.linspace(1, np.shape(gaussian_H)[0],1000), gaussian_H[500,:], 'r')
#plt.xlim(450, 550)
...
block_priors = [0, 0, 0, 0, prior_1,prior_1,prior_1, prior_1, prior_1, prior_1]
...
if subj_id == 'test':   stim_contrast = np.random.permutation(['L','H'])
else: stim_contrast = np.tile(np.random.permutation(['L','H']),len(block_priors)/2)
...
main_exp['trial_reps']  = 50 # 25 * 2 delays = 50 
if subj_id == 'test':
    main_exp['trial_reps']  = 16 # 8 * 2 delays
    sst = False
...
stimList = []
for delay in [3000]: # how many ms 1000 and 3000
        stimList.append({'delay':delay}) #, 'n_reps': n_reps
...
for thisBlock in range(main_exp['nblocks']): # iterate over blocks
    print(thisBlock)
    if subj_id != 'test':  
        instr.block_start(win, thisBlock, main_exp['nblocks'] )
    ...
    contrast_level = stim_contrast[thisBlock][0]     
    if  contrast_level == 'L':
         stim_ID = stim_LC
    else:
         stim_ID = stim_HC
    ...
    for n_trial, thisTrial in enumerate(trials): # iterate over trials
        ...
        while trep:
            ...
            xstim,ystim = exp.toCar(st_prop['radius'] ,angle)  # 1.5 corrects for the different in radius between the cue and the ring
            ...
            if subj_id == 'test': # for demo display stimuli with both contrast levels
                contrast_level = np.random.choice(['L','H'],1)[0]
                if  contrast_level == 'H':
                    stim_ID = stim_HC
                else:
                    stim_ID = stim_LC
           ...
           # delay_frames = 500
                    ...
                    #Eyes['EyesPos'] = np.array([0.5,0.5])
                        if showEyes:
                            ...
                            if  ~np.isnan(Eyes['EyesPos'][0]):
                                eye_pix_coord = (Eyes['EyesPos']-0.5) *np.array([xsize,-ysize])#* res
                                resp.setPos(eye_pix_coord)
                                resp.draw() 
            ...
            for frameN in range(st_prop['FEEDBACK_frames']):
                if kfix == True:
                    ...
                    msgText=visual.TextStim(win=win, ori=0,
                    text= 'MANTEN LOS OJOS EN EL PUNTO DE FIJACION',
                    pos=[0,-2], height=1,
                    color='red',units="cm").draw()
