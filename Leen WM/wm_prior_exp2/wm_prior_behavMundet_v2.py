...
# Stims
fixation = visual.PatchStim(win=win, mask='gauss', size=1.2, pos=[0,0], sf=0, color=[-1,-1,-1],units= "deg")
fixation_ct = visual.PatchStim(win=win, mask='gauss', size=1.2, pos=[0,0], sf=0, color=[-0.75,-0.75,-0.75],units="deg")

stim_LC = visual.Circle(win,radius=st_prop['size_CUE'],pos=[0,0],fillColor=[0.15,0.15,0.15],units= "deg")
stim_HC = visual.Circle(win,radius=st_prop['size_CUE'] ,pos=[0,0],fillColor=[1,1,1],units= "deg")
#stim_HC.draw()
#win.flip()

resp = visual.Circle(win,radius=st_prop['size_CUE'] ,pos=[0,0],fillColor='red',units= "deg")
...
else: stim_contrast = np.tile(np.random.permutation(['L','H']),int(nblocks/2))
